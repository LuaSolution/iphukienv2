...
$(document).ready(function () {
  ...
  function initCarouselModal() {
    ...
    $.post(url, {
      ...
      for (i = 0; i < productInfo["listImage"].length; i++) {
        str +=
          `<div class="carousel-item product-img ${
            productInfo["wishlist"] ? "added-wishlist" : ""
          }" data-img="${productInfo["listImage"][i]}">` +
      ...
      if (productInfo["listImage"].length == 0) {
        str +=
          `<div class="carousel-item product-img ${
            ...
            localStorage.getItem("quickview_image_base_path") +
      ...
      $("#quickview-short-description").html(
        productInfo["product"]["short_description"]
      );
      ...
      if (productInfo["product"]["sale_price"] > 0) {
        $("#quickview-origin").html(
          numberWithCommas(productInfo["product"]["price"]) + "đ"
        );
        $("#quickview-sale").html(
          `${numberWithCommas(
            productInfo["product"]["sale_price"]
          )}đ <span>Giảm ${Math.round(
      ...
      localStorage.setItem(
        "quickview_nhanh_product_id",
        productInfo["product"]["product_id_nhanh"]
      );
      ...
      .done(function (data) {
        ...
        if (chooseProduct.product != null) {
          ...
          if (chooseProduct.product.sale_price > 0) {
            $("#quickview-origin").html(
              numberWithCommas(chooseProduct["product"]["price"]) + "đ"
            );
            $("#quickview-sale").html(
              `${numberWithCommas(
                chooseProduct["product"]["sale_price"]
              )}đ <span>Giảm ${Math.round(
                ((chooseProduct["product"]["price"] -
                  chooseProduct["product"]["sale_price"]) /
                  chooseProduct["product"]["price"]) *
                  100
              )}%</span>`
...
$(document).on("click", ".quickview-btn", function () {
  ...
  localStorage.setItem(
    "quickview_get_child_product_url",
    $(this).data("getchildproducturl")
  );
});
function updateCart() {
  ...
  if (
    currentProduct["listColor"].length > 0 &&
    currentProduct["listSize"].length > 0
  ) {
    if (chooseProduct) {
      console.log(chooseProduct);
      ...
      let cart = localStorage.getItem("ipk_cart")
        ? JSON.parse(localStorage.getItem("ipk_cart"))
        : {};

      if (cart[productid]) {
        cart[productid].quantity =
          parseInt(cart[productid].quantity) + parseInt(quantity);
      } else {
        cart[productid] = {
          color: choosenColor,
          size: choosenSize,
  ...
  if (
    ...
    let quantity =
      $("#quantity-detail").val() == 0 ? 1 : $("#quantity-detail").val();
    let cart = localStorage.getItem("ipk_cart")
      ? JSON.parse(localStorage.getItem("ipk_cart"))
      : {};
